...
import { Croissant, Search } from 'lucide-react';
...
import type { TipoProduccionStock } from '@/features/inventarios/types/inventarios.types';

export interface ProductoStockCantidadItem {
  idProducto: number;
...
interface ProductoStockPickerProps {
  value: ProductoStockCantidadItem[];
  onChange: (value: ProductoStockCantidadItem[]) => void;
  disabled?: boolean;
...
export function ProductoStockPicker({
  value,
  onChange,
  disabled,
  cantidadInicial = 0,
}: ProductoStockPickerProps) {
  const {
    data: productos = [],
    isLoading,
    isError,
  ...
  /* ------------------------------------------------------------------------
     ...
     cada producto que ya está en la operación.
  ------------------------------------------------------------------------ */
  ...
  function actualizarCantidad(
     ...
     * que realmente forman parte de la operación.
     */
    if (cantidadValida === 0) {
      if (!existente) {
        return;
    ...
    onChange([
      ...value,
      {
        ...
        cantidad: cantidadValida,
        tipoProduccion: producto.tipoProduccion as TipoProduccionStock,
        controlarStock: producto.controlarStock ? 1 : 0,
        controlarStockDiario: producto.controlarStockDiario ? 1 : 0,
      },
    ]);
  }
